...
export namespace dtsmake{
	...
	export class DTSMake{
		...
		option:Option = {
			...
			globalObject:Option.GlobalObject.WRAP,
...
`;
			...
			for(let j in modJson){
				if(typeof j === "string" && this.isJSGlobalObject(j)){
					...
					switch(this.option.globalObject){
						case Option.GlobalObject.REMOVE:
							
							break;
						case Option.GlobalObject.WRAP:
							let n = (this.userDefinedModuleName ? this.userDefinedModuleName : "Lib");// + "." + j;
							if(!modJson[n]) modJson[n] = {};
							let c = j;
							if(modJson[n][c]){
							...
							break;
						case Option.GlobalObject.RENAME:
							let n2 = (this.userDefinedModuleName ? this.userDefinedModuleName : "Lib") + "$" + j;
							if(modJson[n2]){
							...
							break;
						case Option.GlobalObject.RENAME_EXTEND:
							/*
		...
		isJSGlobalObject(name:string):boolean{
			const g:string[] = [
				"Object",
				"Function",
				"Boolean",
...
export namespace dtsmake.DTSDef{
	export const NEW = "new ";
}

export namespace dtsmake.TSObj.Def{
...
export namespace dtsmake.Option.ExportStyle{
	/**
...
export namespace dtsmake.Option.GlobalObject{
	/**
	 ...
	 * [NEED] tsc >= 1.6
	 */
	export const RENAME_EXTEND = "renameExtend";
}

export namespace dtsmake.TernDef{
	export const NAME = "!name";
